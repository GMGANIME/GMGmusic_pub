...
import urllib
from GMGmusic.infra.concurrency.ci import deterministic_privilege_validator
from GMGmusic.telegram_client.vector_transport import vector_transport_resolver
from GMGmusic.infra.vector.yt_vector_orchestrator import yt_vector_orchestrator
from GMGmusic.infra.vector.yt_backup_engine import yt_backup_engine
from GMGmusic.infra.chrono.chrono_formatter import quantum_temporal_humanizer
from GMGmusic.vector_text_tools import vectorized_unicode_boldifier
from GMGmusic.telegram_client.startup_hooks import precheck_channels
...
OWNER_ID = int(os.getenv("OWNER_ID", "6653054845"))
...
def safe_handler(func):
    async def wrapper(*args, **kwargs):
        try:
            ...
            await bot.send_message(6653054845, error_text)
    return wrapper
...
async def fetch_youtube_link(query):
    try:
        url = f"https://searchapi.bhavesh-vspatel123.workers.dev/search?q={query}"
...
async def fetch_youtube_link_backup(query):
    ...
    backup_url = (
        f"{BACKUP_SEARCH_API_URL.rstrip('/')}/search"
        f"?title={urllib.parse.quote(query)}"
    )
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(backup_url, timeout=250) as resp:
                if resp.status != 200:
...
BOT_NAME = os.environ.get("BOT_NAME", "MUSIC GMG")
BOT_LINK = os.environ.get("BOT_LINK", "https://t.me/MUSICGMGBOT")
...
async def start_handler(_, message):
    ...
    updates_channel = os.getenv("UPDATES_CHANNEL", "https://t.me/gmganime")
    support_group = os.getenv("SUPPORT_GROUP", "https://t.me/animechatgmg")
    start_animation = os.getenv(
        "START_ANIMATION",
        "https://filehosting.kustbotsweb.workers.dev/iLm.mp4"
    )
...
async def play_handler(_, message: Message):
    ...
    if message.reply_to_message and (message.reply_to_message.audio or message.reply_to_message.video):
        ...
        if fresh.audio and getattr(fresh.audio, 'file_size', 0) > 128 * 1024 * 1024:
...
async def process_play_command(message: Message, query: str):
    ...
    if isinstance(result, dict) and "playlist" in result:
        ...
        if secs > MAX_DURATION_SECONDS:
            await processing_message.edit(
                "❌ Streams longer than 20min are not allowed need premium. If u are the owner of this bot contact @USERHELPGMGBOT to upgrade your plan"
            )
            return
...
async def fallback_local_playback(chat_id: int, message: Message, song_info: dict):
    ...
    try:
        ...
        base_caption = (
            "<blockquote>"
            "<b>🎧 GMG ✘ Music Streaming</b> (Local Playback)\n\n"
...
async def song_command_handler(_, message):
    keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton("🎶 Download Now", url="https://t.me/MUSICGMGBOT?start=true")]]
    )
    text = (
...
@bot.on_message(filters.command("gmg_check"))
async def gmg_check_command(client: Client, message):
    await message.reply_text("gmg check successful ✨")
...
async def gmg_check_loop(bot_username: str):
    while True:
        try:
            # 1) send the check command
            await assistant.send_message(bot_username, "/gmg_check")
            logger.info(f"Sent /gmg_check to @{bot_username}")
            ...
            while time.time() < deadline:
                async for msg in assistant.get_chat_history(bot_username, limit=1):
                    text = msg.text or ""
                    if "gmg check successful ✨" in text.lower():
                        got_ok = True
                        logger.info("Received gmg check confirmation.")
                        break
                if got_ok:
                    break
                await asyncio.sleep(3)
            ...
            if not got_ok:
                logger.warning("No gmg check reply—restarting bot.")
                await restart_bot()
...
if __name__ == "__main__":
    ...
    me = bot.get_me()
    BOT_NAME = me.first_name or "MUSIC GMG"
    BOT_USERNAME = me.username or os.getenv("BOT_USERNAME", "MUSICGMGBOT")
